...
interface ForgotPasswordProps {
    onOTPNavigate: (email: string) => void;
    onLoginNavigate: () => void;
}

const ForgotPassword: React.FC<ForgotPasswordProps> = ({ onOTPNavigate, onLoginNavigate }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSuccess(true);
            // Wait a bit then navigate to OTP page
            setTimeout(() => {
                onOTPNavigate(email);
            }, 1500);
        } catch (err: any) {
        ...
        <div className="flex items-center justify-center min-h-[70vh] animate-in fade-in zoom-in-95 duration-500">
            <div className="bg-cream-light/50 backdrop-blur-md border border-white/30 rounded-[2.5rem] p-8 md:p-12 shadow-inner w-full max-w-md mx-auto transition-all hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)]">
                <div className="text-center mb-10">
                    ...
                    <p className="text-[10px] font-black opacity-60 uppercase tracking-widest text-slate-600">Enter your email to receive a reset code</p>
                </div>
